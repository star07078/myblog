<template>
    <div class="paginataion-container">
        ...
        <img 
			...
			@click='numLower'
		/>
        <input @focus="focus" @blur="blur" type="number" v-model='num' class="current-page" />
...
<script>
    export default {
		...
		computed: {
			page(){
				return this.$store.state.fineList.page
			},
			len(){
				return this.$store.state.fineList.len
			},
			type(){
		...
		methods: {
			...
			numLower(){
				...
				this.once = true;
				sessionStorage.setItem('fineNum',this.num)
				this.$axios({
					url: this.$store.state.love,
					params: {
						page: this.num
					}
				})
		...
		created(){
			var n = sessionStorage.getItem('fineNum');
			if(n){
				this.num = n;
			}
		}
