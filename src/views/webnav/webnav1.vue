    ...
    <div class="blog-container">
		<ul class="label-list wn-list" >
			<li v-for="(nav,index) in navs"
				...
				v-on:click="a(index,nav.name)"
			>
				<a href="javascript:void(0);" class="tab-link button" >
					{{ nav.name }}
				</a>
			</li>
		</ul>
		<div class="blog-list-container">
			<div class="webnav about-me bga-back-top">
				<div class="wn-box"
					v-for="(content, index) in contents.list"
					:key="index"
				>
					<!-- <h1>{{content.list}}</h1> -->
					<!-- <div class="wn-container" v-for="(wnlift,i) in content.html" :key="i"> -->
					<div class="wn-container">
						<div class="wn-l">
							<span>{{content.title}}</span>
						</div>
						<div class="wn-r">
							<a :href="'http://' + item.http "
								target="_blank" v-for="(item,n) in content.html"
								:key="n">
							{{item.name}}
							</a>
						</div>
...
<script>
	export default {
			...
			return {
				...
				{ h1: '圈子',
					...
					},
					{
						...
						{ http: 'www.runoob.com', name: '菜鸟教程' },{ http: 'developer.mozilla.org/zh-CN/', name: 'Mozilla' },
		...
		methods: {
			a(str,name){
				this.indexs = str;
				this.$axios.get('http://lishaokai.cn:8080/webnav/list?name='+name)
				.then(item=>{
					this.$store.commit('setWebnavList',item.data)
				})
			}
		},
		computed: {
			navs(){
				return this.$store.state.webnavType.list;
			},
			contents(){
				return this.$store.state.webnavList;
			}
		}
	}
</script>
